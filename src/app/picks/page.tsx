...
export default function PicksPage() {
  ...
  async function loadData(majorId: MajorId) {
    setLoading(true); setError(""); setActiveSlot(null); setSaved(false);
    try {
      ...
      // Load existing picks — match by ID or name fallback
      ...
      myMajorPicks.forEach((p: any, i: number) => {
        if (i >= 5) return;
        const match = golfers.find(g => g.id === p.golferId || g.displayName.toLowerCase() === (p.golferName ?? "").toLowerCase());
        loadedPicks[i] = match ?? { id: p.golferId, displayName: p.golferName, tier: p.tier ?? "field", majorId };
      });
      setPicks(loadedPicks);
      setSaved(myMajorPicks.length > 0);

      const used: string[] = [];
      MAJORS.forEach(m => { if (m.id !== majorId) (entryData.majors?.[m.id]?.picks ?? []).forEach((p: any) => { if (p.golferId) used.push(p.golferId); }); });
  ...
  function clearSlot(i: number) { const next = [...picks]; next[i] = null; setPicks(next); setSaved(false); }
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch("/api/picks/submit", {
        ...
        body: JSON.stringify({ entryId: session!.entryId, majorId: activeMajor, picks: picks.map((p, i) => ({ golferId: p!.id, golferName: p!.displayName, isTopPick: i === 0, tier: p!.tier })) })
      });
    ...
    <div style={{ minHeight: "100vh", background: "var(--fairway-dark)" }}>
      ...
      <div style={{ maxWidth: 920, margin: "0 auto", padding: "32px 20px" }}>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: "0.85rem", marginBottom: 28 }}>
          Select 5 golfers per major. <strong style={{ color: "#facc15" }}>Slot 1 = Top Pick</strong> — earns bigger bonus if they win. Cannot reuse a golfer across majors.
        </p>
        <div style={{ display: "flex", gap: 6, marginBottom: 28, flexWrap: "wrap" }}>
          {MAJORS.map(m => (
            <button key={m.id} onClick={() => setActiveMajor(m.id)} style={{ padding: "7px 16px", borderRadius: 20, border: `1px solid ${activeMajor === m.id ? "var(--green-400)" : "var(--border)"}`, background: activeMajor === m.id ? "rgba(77,189,136,0.12)" : "transparent", color: activeMajor === m.id ? "var(--green-400)" : "var(--text-muted)", fontFamily: "'DM Sans', sans-serif", fontSize: "0.83rem", fontWeight: activeMajor === m.id ? 600 : 400, cursor: "pointer" }}>
        ...
        {loading ? (
          <div style={{ textAlign: "center", padding: "60px 0", color: "var(--text-muted)" }}><div style={{ fontSize: "2rem", marginBottom: 12 }}>⛳</div><p style={{ fontFamily: "'DM Mono', monospace" }}>Loading field…</p></div>
        ) : field.length === 0 ? (
          <div style={{ textAlign: "center", padding: "60px 0" }}><div style={{ fontSize: "3rem", marginBottom: 16 }}>📋</div><p style={{ color: "var(--text-muted)" }}>Field not published yet. Check back soon.</p></div>
        ) : (
          <>
            {majorInfo?.pickDeadline && (
              <div style={{ background: locked ? "rgba(239,68,68,0.08)" : "rgba(77,189,136,0.08)", border: `1px solid ${locked ? "rgba(239,68,68,0.25)" : "rgba(77,189,136,0.25)"}`, borderRadius: 8, padding: "10px 16px", marginBottom: 16, color: locked ? "#f87171" : "var(--green-400)", fontSize: "0.82rem", fontFamily: "'DM Mono', monospace" }}>
                {locked ? "🔒 Picks are locked" : `⏰ Deadline: ${new Date(majorInfo.pickDeadline).toLocaleString("en-US", { timeZone: "America/Chicago", dateStyle: "full", timeStyle: "short" })} CT`}
              </div>
            )}
            {saved && !locked && (
              <div style={{ background: "rgba(77,189,136,0.1)", border: "1px solid rgba(77,189,136,0.3)", borderRadius: 8, padding: "10px 16px", marginBottom: 16, color: "var(--green-400)", fontSize: "0.85rem" }}>
                ✓ Your picks are saved. You can update them any time before the deadline.
              </div>
            )}

            {/* Bonus chart */}
            <div className="card" style={{ padding: "14px 18px", marginBottom: 24 }}>
              <div style={{ color: "var(--text-muted)", fontSize: "0.7rem", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 10, fontWeight: 600 }}>Bonus Chart</div>
              ...
              <div>
                ...
                {picks.map((pick, i) => (
                  <div key={i} style={{ marginBottom: 8 }}>
                    <div style={{ color: i===0?"#facc15":"var(--text-muted)", fontSize: "0.68rem", fontFamily: "'DM Mono', monospace", marginBottom: 3 }}>{i===0?"⭐ SLOT 1 — TOP PICK":`SLOT ${i+1}`}</div>
                    <div onClick={() => { if (!locked) { setActiveSlot(activeSlot===i?null:i); setSearch(""); } }}
                      style={{ padding: "10px 14px", borderRadius: 8, cursor: locked?"default":"pointer", border: `1px solid ${activeSlot===i?"var(--green-400)":pick?(i===0?"rgba(250,204,21,0.35)":"var(--border)"):"rgba(255,255,255,0.08)"}`, background: activeSlot===i?"rgba(77,189,136,0.07)":pick?"rgba(17,45,28,0.8)":"rgba(10,31,20,0.3)", display: "flex", alignItems: "center", justifyContent: "space-between", minHeight: 48 }}>
                      {pick ? (
                        <>
                          <div>
                          ...
                          {!locked && <button onClick={e => { e.stopPropagation(); clearSlot(i); }} style={{ background: "none", border: "none", color: "#6b7280", cursor: "pointer", fontSize: "1.1rem", padding: "0 4px" }}>×</button>}
                        </>
                      ) : (
                        <span style={{ color: "var(--text-muted)", fontSize: "0.83rem" }}>{locked?"—":activeSlot===i?"Choose from field →":"Click to select"}</span>
                      )}
                    </div>
                  </div>
                ))}
                {topPick && (
                  <div style={{ marginTop: 14, background: "rgba(250,204,21,0.05)", border: "1px solid rgba(250,204,21,0.18)", borderRadius: 8, padding: "10px 14px" }}>
                    ...
                    <div style={{ color: "var(--text-secondary)", fontSize: "0.8rem" }}>Top Pick bonus: <strong style={{ color: "#facc15" }}>{ODDS_BONUSES[topPick.tier]?.topPickBonus} strokes</strong></div>
                  </div>
                )}
                {!locked && (
                  <div style={{ marginTop: 18 }}>
                    {error && <div style={{ color: "#f87171", fontSize: "0.8rem", marginBottom: 8 }}>{error}</div>}
                    <button className="btn-primary" style={{ width: "100%", padding: "12px" }} onClick={handleSubmit} disabled={!picks.every(Boolean)||saving}>
              ...
              <div>
                ...
                <div style={{ maxHeight: 480, overflowY: "auto", display: "flex", flexDirection: "column", gap: 3 }}>
                  {filteredField.map(g => {
                    ...
                    return (
                      <div key={g.id} onClick={() => !unavail && !locked && activeSlot!==null && selectGolfer(g)}
                        style={{ padding: "8px 12px", borderRadius: 7, border: `1px solid ${isSel?"rgba(77,189,136,0.4)":"var(--border)"}`, background: isSel?"rgba(77,189,136,0.07)":unavail?"transparent":"rgba(17,45,28,0.4)", opacity: unavail?0.35:1, cursor: unavail||locked||activeSlot===null?"default":"pointer", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
