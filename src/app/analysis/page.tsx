...
function StatCard({ label, value, sub, color }: { label: string; value: string | number; sub?: string; color?: string }) {
  return (
    <div style={{ background: "rgba(15,32,64,0.6)", border: "1px solid var(--border)", borderRadius: 10, padding: "16px 20px" }}>
...
export default function AnalysisPage() {
  ...
  const [majorId, setMajorId] = useState("british-open");
  ...
  const beforeDeadline = data?.pickDeadline
    ? Date.now() < new Date(data.pickDeadline).getTime()
    : false;

  const deadlineFormatted = data?.pickDeadline
    ? new Date(data.pickDeadline).toLocaleString("en-US", {
    ...
    <div style={{ minHeight: "100vh", background: "var(--fairway-dark)" }}>
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "32px 20px" }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {["masters","pga","us-open","british-open"].map(mid => (
              <button key={mid} onClick={() => setMajorId(mid)} style={{
                ...
                background: majorId===mid?"rgba(240,192,64,0.12)":"transparent",
        ...
        {loading ? (
          ...
          <>
            {/* Always-visible summary stat cards */}
            ...
            {beforeDeadline ? (
              /* Pre-deadline placeholder */
              <div style={{ background: "rgba(15,32,64,0.7)", border: "1px solid var(--border)", borderRadius: 12, padding: "48px 32px", textAlign: "center" }}>
                <div style={{ fontSize: "2rem", marginBottom: 16 }}>🔒</div>
                <div style={{ color: "#f0faf4", fontSize: "1rem", fontWeight: 600, marginBottom: 8 }}>
                  Pick analysis available after the submission deadline
                </div>
                <div style={{ color: "var(--text-muted)", fontSize: "0.85rem", fontFamily: "'DM Mono', monospace" }}>
                  {deadlineFormatted
                    ? `Picks hidden until ${deadlineFormatted}`
                    : "Picks hidden until the submission deadline passes"}
                </div>
              </div>
            ) : (
              <>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12, marginBottom: 28 }}>
                  {[
                    ...
                    <div key={label} style={{ background: "rgba(15,32,64,0.7)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px 18px" }}>
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      ...
                      {[
                        ...
                        <button key={opt.key} onClick={() => setSortBy(opt.key)} style={{
                          ...
                          background: sortBy===opt.key?"rgba(240,192,64,0.12)":"transparent",
                      ...
                      return (
                        <div key={g.name} style={{
                          ...
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 4, minWidth: 140 }}>
                            ...
                            <div style={{ width: `${pickShare}%`, maxWidth: 60, minWidth: 4, height: 3, background: "rgba(240,192,64,0.4)", borderRadius: 2 }} />
                          </div>
